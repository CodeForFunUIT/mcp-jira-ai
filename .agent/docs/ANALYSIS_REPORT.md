...
graph TD
    ...
    Q2 -->|JSON files| SS[".vnpt-dev-agent/sessions/"]
...
| `suggest_branch_name` | Sinh gợi ý tên branch theo chuẩn Conventional: `feature/VNPTAI-123-add-user-profile` |
...
| `load_session` | Khôi phục context khi user nói "tiếp tục VNPTAI-123" |
| `list_sessions` | Xem danh sách tất cả sessions đang active, sorted by updated time |

**Storage:** `.vnpt-dev-agent/sessions/<issueKey>.json`
...
8. **Session persistence** — Context được lưu giữa các phiên chat. User nói "tiếp tục VNPTAI-123" → AI load toàn bộ context cũ.
